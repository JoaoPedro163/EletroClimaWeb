        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                let deletar = $('<button>',{
                    ...
                    click: function(){
                        deletarModelo(meuModelo.id);
                    }
                });

                let acoes = $('<td>').append(deletar);
                
                let tr = $('<tr>').attr('data-id', meuModelo.id).append(imagem).append(modelo).append(ambiente).append(capacidade).append(acoes);
...
function deletarModelo(id) {
    $.ajax({
        url: 'http://localhost:8080/modelos/deletar-modelo/' + id,
        method: 'DELETE',
        success: function () {
            carregarMeusModelos();
